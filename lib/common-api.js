      ...
      this.authenticate(this.authenticatedData, (err, authData) => {
        if (err) {
          if (retryCredits > 0) { // retry one more time
            console.log('Authentication failed. Retrying ' + retryCredits + ' times left.')
            return tryToAuthenticate(retryCredits - 1, callDistant, streamParser, resolve, reject)
          } else {
            console.log('Authentication failed. No retry credits left.')
            return reject(err)
          }
        } else {
