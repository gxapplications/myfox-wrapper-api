...
export default function (wrapperApi) {
  ...
  trumpetParser.selectAll('footer', () => {
    ...
    for (let key in trumpetParser.domotic) {
      if (wrapperApi.persistentStates.domotic &&
          wrapperApi.persistentStates.domotic.value &&
          wrapperApi.persistentStates.domotic.value[key] &&
    ...
    // data can contain more than the /home parser values. So keep them.
    if (wrapperApi.persistentStates.data && wrapperApi.persistentStates.data.value) {
      const dataValue = assignDeep({}, wrapperApi.persistentStates.data.value, trumpetParser.data)
