...
export function step4Parser (wrapperApi) {
  ...
  trumpetParser.selectAll(
    'form#scenarioForm div.tab-scenario span.input-checkbox',
    trumpetAttr('data-default', (dataDefault) => {
...
export function step3TempFixer (payload, settings) {
  ...
  // DELTA to apply: operator 0 : -0.5°  ;  operator 1 : +0.5°  ;  operator 2 : +0°
  settings.forEach((setting) => {
    const {toTemperature, controls} = setting
    controls.forEach((control) => {
      let delta = 0
      switch (control.condition) {
        case 'trigger_4':
          if (payload['_trigger[4][1][deviceSlaveId]'] !== control.deviceSlaveId) {
            // TODO !1: big error to throw, can happens only if scenario changed and not dashboard settings. And a test for it
          }
          delta = (payload['_trigger[4][1][operator]'] === '1') ? 0.5 : -0.5
          payload['_trigger[4][1][value]'] = Number(toTemperature) + delta + ''
          break
        case 'condition_4_1':
          if (payload['_condition[4][1][deviceSlaveId]'] !== control.deviceSlaveId) {
            // TODO !1: big error to throw, can happens only if scenario changed and not dashboard settings. And a test for it
          }
          delta = (payload['_condition[4][1][operator]'] === '1')
              ? 0.5 : ((payload['_condition[4][1][operator]'] === '0') ? -0.5 : 0)
          payload['_condition[4][1][value]'] = Number(toTemperature) + delta + ''
          break
        case 'condition_4_2':
          if (payload['_condition[4][2][deviceSlaveId]'] !== control.deviceSlaveId) {
            // TODO !1: big error to throw, can happens only if scenario changed and not dashboard settings. And a test for it
          }
          delta = (payload['_condition[4][2][operator]'] === '1')
              ? 0.5 : ((payload['_condition[4][2][operator]'] === '0') ? -0.5 : 0)
          payload['_condition[4][2][value]'] = Number(toTemperature) + delta + ''
          break
        default:
          // TODO !2: big error to throw, should never happens, so prefer to crash scenario update! And a test for it
      }
    })
  })
  return payload
