...
export function httpsRequest (method, path, streamParser, callback, queryParams, payload, headers, cookieJar) {
  ...
  try {
    ...
    let req = ((config.get('html.myfox.protocol') === 'http') ? http : https).request(requestData, (res) => {
      ...
      codeKo200Instance.on('error', () => {
        // Nothing to do: sometimes it's not JSON data!
      })
      ...
      if (streamParser !== null && streamParser !== undefined) {
        ...
        unzippedRes.on('end', callback) // TODO !1: c'est unzippedRes qui doit aller jusq'uà end() ? ou c'est mieux de faire streamParser.end() ? (donc peut-etre plus tôt) L'UN OU L'AUTRE !
      } else {
...
// TODO !1: TU
/**
 * This function returns a function to generate a stream parser for a given HTML element, passed to the given callback.
 *
