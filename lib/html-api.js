...
import domoticActionParser from './html-parsers/domotic-action'
import heatingActionParser from './html-parsers/heating-action'
import alarmActionParser from './html-parsers/alarm-action'
...
class MyfoxWrapperApiHtml extends MyfoxWrapperApiCommon {
  ...
  _callScenarioAction ({id, action, delay = 0}, callback, macroId = null, ...nextActions) {
    ...
    if (delay > 0) {
      ...
      this.callApi(url, (action === 'play') ? 'GET' : 'POST', scenarioActionParser(this), {}, {}, {})
        .then((parser) => {
  ...
  _callDomoticAction ({id, action, delay = 0}, callback, macroId = null, ...nextActions) {
    ...
    if (delay > 0) {
      ...
      this.callApi(url, 'POST', domoticActionParser(this), {}, {}, {})
        .then((parser) => {
          if (parser.status === 'ok') {
  ...
  _callHeatingAction ({id, action, delay = 0}, callback, macroId = null, ...nextActions) {
    ...
    if (delay > 0) {
      ...
      this.callApi(url, 'POST', heatingActionParser(this), {}, {}, {})
        .then((parser) => {
   ...
   * @param  {string}   [macroId]    Optional ID to keep track of the action if multiple steps or delay is provided.
   */
  callAlarmLevelAction ({action}, callback, macroId = undefined) {
    // Validation of parameters
    const validator = Joi.object({
    ...
    this.callApi(url, 'GET', alarmActionParser(this), {}, {}, {})
      .then((parser) => {
        // If action succeed, then must update persistentStates
        if (parser.status === 'ok') {
