...
class MyfoxWrapperApiHtml extends MyfoxWrapperApiCommon {
  ...
  _callScenarioAction ({id, action, delay = 0}, callback, macroId = null, ...nextActions) {
    ...
    if (delay > 0) {
      ...
      const that = this
      setTimeout(function () {
        that._callScenarioAction({id, action, delay: 0}, that.notifyMacroListeners, macroId, ...nextActions)
      }, delay)
      ...
      this.callApi(url, (action === 'play') ? 'GET' : 'POST', scenarioActionParser(this), {}, {}, {})
        .then((parser) => {
          ...
          if (nextActions.length > 0) {
            ...
            this._callScenarioAction(nextAction, this.notifyMacroListeners, macroId, ...nextActions)
          }
        })
        .catch((err) => {
          console.error(err)
