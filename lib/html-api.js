...
class MyfoxWrapperApiHtml extends MyfoxWrapperApiCommon {
   ...
   */
  callHome (callback) {
    super.callApi(config.get('html.myfox.paths.home'), 'GET', homeParser(this), {}, {}, {})
      .then((parser) => {
        callback(null, {
  ...
  callScenarioAction ({id, action, delay = 0}, callback, ...nextActions) {
    ...
    if (delay > 0 || nextActions.length > 0) {
      ...
      this._callScenarioAction({id, action, delay}, callback)
    }
  }
  ...
  _callScenarioAction ({id, action, delay = 0}, callback, macroId = null, ...nextActions) {
    ...
    if (delay > 0) {
      ...
      setTimeout(() => {
        this._callScenarioAction({id, action, delat: 0}, super.notifyListeners, macroId, ...nextActions)
      }, delay)
      callback(null, { id: macroId, state: 'delayed', remaining: nextActions.length })
    } else {
      let url = config.get('html.myfox.paths.scenario_action').replace('{action}', action).replace('{id}', id)
      if (action === 'play') {
        url += '?_=' + new Date()
      }
      super.callApi(url, (action === 'play') ? 'GET' : 'POST', scenarioActionParser(this), {}, {}, {})
        .then((parser) => {
          ...
          if (nextActions.length > 0) {
            ...
            this._callScenarioAction(nextAction, super.notifyListeners, macroId, ...nextActions)
          }
        })
        .catch((err) => {
          console.error(err)
